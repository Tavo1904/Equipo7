...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['accion'] ?? '') === 'entrada_stock') {
    ...
    try {
        ...
        $stmtMov = $conexion->prepare("INSERT INTO movimientos_inventario (id_producto, id_usuario, tipo_movimiento, cantidad, observaciones) VALUES (?, ?, 'entrada', ?, ?)");
        ...
        try {
            ...
            if ($id === '') {
                $sql = "INSERT INTO productos
                    (nombre, compuesto, numero_lote, categoria, laboratorio, presentacion, fecha_caducidad, precio, precio_mayoreo, costo_adquisicion, stock, stock_minimo, descripcion, estado)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'disponible')";
                ...
                $stmtMov = $conexion->prepare("INSERT INTO movimientos_inventario (id_producto, id_usuario, tipo_movimiento, cantidad, observaciones) VALUES (?, ?, 'entrada', ?, 'Inventario inicial')");
                ...
                $sql = "UPDATE productos SET
                    nombre=?, compuesto=?, numero_lote=?, categoria=?, laboratorio=?, presentacion=?,
                    fecha_caducidad=?, precio=?, precio_mayoreo=?, costo_adquisicion=?, stock_minimo=?, descripcion=?
                    WHERE id_producto=?";
                $stmt = $conexion->prepare($sql);
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h3 class="fw-bold text-dark">Gestion de Catalogo e Inventario</h3>
        ...
        <table class="table table-hover align-middle">
            ...
            <tbody>
                <?php foreach ($productos as $p):
                    ...
                    $alertClass = $expiry['status'] === 'ok' ? 'bg-success-subtle text-success' : ($expiry['status'] === 'warning' ? 'bg-warning-subtle text-warning' : 'bg-danger-subtle text-danger');
                ?>
                <tr>
                    <td class="fw-bold"><?php echo h($p['nombre']); ?><div class="small text-muted"><?php echo h($p['presentacion']); ?></div></td>
                    ...
                    <td>
                        ...
                        <?php if ($expiry['message']): ?><div class="badge <?php echo $alertClass; ?> border mt-1"><?php echo h($expiry['message']); ?></div><?php endif; ?>
                    </td>
                    <td class="text-center fw-bold"><?php echo (int)$p['stock']; ?></td>
                    <td class="fw-bold text-primary">$<?php echo number_format((float)$p['precio'], 2); ?></td>
                    <td class="fw-bold text-success">$<?php echo number_format((float)$p['precio_mayoreo'], 2); ?></td>
                    <td class="text-end">
                        <button class="btn btn-sm btn-success" onclick='abrirModalEntrada(<?php echo json_encode($p); ?>)' title="Reabastecer"><i class="bi bi-plus-lg"></i></button>
                        <button class="btn btn-sm btn-warning" onclick='abrirModalEditar(<?php echo json_encode($p); ?>)' title="Editar"><i class="bi bi-pencil"></i></button>
                        <?php if ($isAdmin): ?>
                            <a href="inventario.php?borrar=<?php echo (int)$p['id_producto']; ?>" class="btn btn-sm btn-danger" onclick="return confirm('Confirmar eliminacion del producto');"><i class="bi bi-trash"></i></a>
                        <?php endif; ?>
                    </td>
                </tr>
                <?php endforeach; ?>
