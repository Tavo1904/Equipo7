...
// Clientes para selector general
    ...
    <div class="col-md-8 pe-3">
        <div class="d-flex justify-content-between align-items-center mb-3 pt-2">
            <div>
                ...
                <small class="text-muted">Busqueda por nombre, acentos o lote en menos de 1 segundo</small>
            </div>
            ...
            <div class="col item-card">
                <div class="card h-100 card-custom border-0 shadow-sm position-relative overflow-hidden product-card-btn"
                    ...
                    <div class="card-body text-center p-3 d-flex flex-column justify-content-center">
                        ...
                        <?php if ($expiry['status'] === 'warning'): ?><span class="badge bg-warning-subtle text-warning border mt-2"><?php echo h($expiry['message']); ?></span><?php endif; ?>
        ...
        <div class="card card-custom border-0 shadow-lg overflow-hidden">
            ...
            <div class="p-3 border-bottom bg-light">
                ...
                <select class="form-select" id="clienteSelect" onchange="actualizarCliente()">
                    <option value="" data-tipo="minorista" data-limite="50">Venta de mostrador</option>
                    <?php foreach ($clientes as $c): ?>
                        <option value="<?php echo (int)$c['id_usuario']; ?>" data-tipo="<?php echo h($c['tipo_cliente']); ?>" data-limite="<?php echo (int)$c['limite_mayoreo']; ?>">
            ...
            <!-- Footer fijo con totales y cobro -->
            <div class="card-footer bg-light border-top p-3">
                ...
                <!-- Panel credito (se muestra solo cuando tipo_pago = credito) -->
...
<script>
let carrito = [];
let clienteActual = { id: null, tipo: 'minorista', limite: 50 };

function precioAplicable(item) {
    if (item.modalidad === 'mayoreo') return item.precio_mayoreo;
    return item.precio;
}
...
function agregarAlCarrito(prod) {
    ...
    const modalidad = clienteActual.tipo === 'mayorista' && 1 >= clienteActual.limite ? 'mayoreo' : 'menudeo';
    carrito.push({
        id: prod.id, nombre: prod.nombre, lote: prod.lote,
        precio: Number(prod.precio),
        precio_mayoreo: Number(prod.precio_mayoreo || prod.precio),
        cantidad: 1, stockMax: Number(prod.stock), modalidad
    });
...
function aplicarReglasMayoreo() {
    carrito.forEach(item => {
        if (clienteActual.tipo === 'mayorista' && item.cantidad >= clienteActual.limite) {
...
function renderizarCarrito() {
    ...
    tbody.innerHTML = '';
    let total = 0, itemsTotal = 0, mensajes = [];
    ...
    carrito.forEach((item, index) => {
        ...
        if (clienteActual.tipo === 'mayorista' && item.cantidad < clienteActual.limite)
            mensajes.push(`Descuento mayorista aplica desde ${clienteActual.limite} uds. para ${item.nombre}.`);
        ...
        tbody.innerHTML += `
            <tr>
                <td class="ps-3 align-middle py-2">
                    ...
                    <select class="form-select form-select-sm mt-1" onchange="cambiarModalidad(${index}, this.value)">
                        <option value="menudeo" ${item.modalidad==='menudeo'?'selected':''}>Menudeo $${item.precio.toFixed(2)}</option>
                        <option value="mayoreo" ${item.modalidad==='mayoreo'?'selected':''}>Mayoreo $${item.precio_mayoreo.toFixed(2)}</option>
                    </select>
                </td>
                ...
                <td class="text-end pe-2 align-middle">
                    ...
                    <div class="small text-muted">$${unitario.toFixed(2)} c/u</div>
                </td>
    ...
    mayoreoMsg.classList.toggle('d-none', mensajes.length === 0);
    mayoreoMsg.innerText = mensajes[0] || '';

    // Validar credito en tiempo real si está seleccionado
...
function cambiarCant(index, delta) {
    ...
    if (nuevaCant <= 0) { eliminar(index); } else { item.cantidad = nuevaCant; aplicarReglasMayoreo(); renderizarCarrito(); }
}
...
function actualizarCliente() {
    ...
    clienteActual = {
        ...
        limite: Number(option.dataset.limite || 50)
    };
    aplicarReglasMayoreo();
    renderizarCarrito();
}
...
function onCreditoClienteChange() {
    ...
    const saldoEl = document.getElementById('saldoCreditoDisp');
    if (sel.value) {
...
function validarCreditoEnTiempoReal(total) {
    ...
    const saldo = Number(sel.selectedOptions[0].dataset.saldo);
    if (total > saldo) {
        msg.innerText = `Credito insuficiente. Saldo disponible: $${saldo.toFixed(2)} — Total: $${total.toFixed(2)}`;
...
async function cobrar() {
    ...
    if (tipoPago === 'credito') {
        const sel = document.getElementById('clienteCreditoSelect');
        if (!sel || !sel.value) {
            alert('Selecciona un cliente con credito disponible.');
            return;
        }
        idClienteCredito = parseInt(sel.value);
        const saldo = Number(sel.selectedOptions[0].dataset.saldo);
        if (total > saldo) {
            alert('Credito insuficiente. Saldo disponible: $' + saldo.toFixed(2) + '\nTotal de la venta: $' + total.toFixed(2));
            return;
        }
    }
